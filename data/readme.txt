...
Description: A flat-file containing data on all biological specimens collected between 2017 and 2020 to examine the food web structure of Northwestern Montana lakes. One row is one unique combination of `sample_id` and `isotope_id`. A biological sample is either (a) one individual fish (b) all individuals of a macroinvertebrate taxon from a unique combination of lake, transect, depth, date (c) bulk periphyton from a unique combination of lake, date (d) bulk zooplankton (e) other (e.g., a handful of larch needles). Not all specimens were analyzed for d13C and d15N (i.e., some `sample_id`s do not have an `isotope_id` nor d13c and d15n). As of June 2020, samples were frozen for long-term storage in the indoor/outdoor storage near Flathead Lake Biological Station's lab (Polson, MT).
...
method_of_collection: str. The method by which the specimen was captured afield.
...
sample_net_weight_mg: float. The mass of specimen biological material sent to the lab for isotope analysis.
...
total_c_ug: float. Total carbon of the specimen. Micrograms.
total_n_ug: float. Total nitrogen of the specimen. Micrograms.
...
Derived fields for isotope samples: values calculated from UC Davis's d13C and d15N and used to make inferences in our study system. 
...
d13c_baseline_corrected: float. The baseline-corrected d13C of the specimen. Calculated via `d13c` - `lake_d13c_baseline_mean` = `d13c_baseline_corrected`. Permille.
d15n_baseline_corrected: float. The baseline-corrected d15N of the specimen. Calculated via `d15n` - `lake_d15n_baseline_mean` = `d15n_baseline_corrected`. Permille.