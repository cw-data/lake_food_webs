...
Description: `food_web_2020.xlsx` is a two-sheet spreadsheet containing data collected between 2017 and 2020 to examine the food web structure of Northwestern Montana lakes. Data on biological specimens is in the "food_web_data" sheet, secchi readings are in the "secchi" sheet.

The "key field" for "food_web_data" is (`sample_id`, `isotope_id`) (i.e., one row in the flat file is one unique combination of `sample_id` and `isotope_id`). A biological specimen in this file is either (a) one individual fish (b) all individuals of a macroinvertebrate taxon from a unique combination of lake, transect, depth, date (c) bulk periphyton from a unique combination of lake, date (d) bulk zooplankton from a unique combination of lake, date (e) other (e.g., a handful of larch needles). Not all specimens were analyzed for d13C and d15N, which means that some `sample_id`s do not have an `isotope_id` nor d13c and d15n. As of June 2020, specimens were frozen for long-term storage in the indoor/outdoor storage beside Flathead Lake Biological Station's lab (Polson, MT).
...
`lake`: str. Name of the lake at which the secchi reading was collected.
`latitude`: float. Latitude at which the secchi reading was collected. Decimal degrees north.
`longitude`: float. Longitude at which the secchi reading was collected. Decimal degrees east.
`water_temp_c`: int. The lake surface water temperature where the secchi reading was collected. Degrees celsius.
`secchi_m`: float. The depth at which the alternating black-and-white pattern became indistinguishable on the 20-cm diameter circular secchi disk. Meters.
`cloud_cover`: chr. Description of sky cover at time of secchi reading.
`wind_mph`: chr. Estimate of wind at time of secchi reading. Miles per hour.
`datetime`: chr. Datetime the secchi reading was collected. yyyy-mm-dd hh:mm:ss.